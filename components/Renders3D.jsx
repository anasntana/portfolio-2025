...
const Renders3D = () => {
  ...
  const images = Array.from({ length: 12 }, (_, i) => `/3D/3D_${i + 1}.png`);
  ...
  return (
    <div className="items-center flex flex-col">
      <div className="h-96 max-w-96 flex flex-col justify-center items-center text-center">
        <h1 className="text-4xl">3D Renders</h1>
        <p>I bring concepts to life with 3D renders.</p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 px-8 md:px-20 my-20 w-full">
        {images.map((src, i) => (
          ...
          >
            <img src={src} alt={`3D Render ${i + 1}`} className="w-full h-full object-cover" />
          </div>
        ))}
      </div>
